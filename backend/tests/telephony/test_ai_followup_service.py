...
def test_initiate_followup_preserves_case_and_creates_records():
    ...
    # Create a unique test phone number so repeated test runs
    # do not violate the customers_phone_key constraint.
    customer_phone = f"010{uuid4().int % 10**8:08d}"
    ...
    mock_resp = {
        "uuid": "mock-vonage-uuid-1",
        "status": "started",
    }

    with patch(
        "app.services.ai_followup_service.create_outbound_call",
        ...
        followups = (
            db.query(AIFollowup)
            .filter(AIFollowup.case_id == case.id)
            .all()
        )
