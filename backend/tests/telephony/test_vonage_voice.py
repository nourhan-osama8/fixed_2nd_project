...
def test_ncco_builders():
    ...
    greeting_ncco = build_greeting_ncco(speech_webhook_url=speech_url, greeting_text="مرحبا")
...
def test_vonage_create_outbound_call():
    ...
    mock_voice.create_call.return_value = {"uuid": "vonage-call-uuid-1234", "status": "started"}

    mock_client = MagicMock()
    mock_client.voice = mock_voice

    with patch("app.telephony.vonage.client.get_vonage_client", return_value=mock_client):
...
def test_vonage_answer_webhook(client):
    ...
    resp = client.get("/api/v1/telephony/vonage/answer?followup_id=00000000-0000-0000-0000-000000000001&call_id=00000000-0000-0000-0000-000000000002")
...
def test_vonage_speech_webhook_yes(client):
    ...
    customer = Customer(name="Vonage Test User 1", phone="01011112222")
    db.add(customer)
    db.commit()

    case = Case(customer_id=customer.id, issue="مشكلة الراوتر", status=CaseStatus.AI_FOLLOW_UP_SCHEDULED)
    db.add(case)
    db.commit()
    ...
    payload = {
        "speech": {
            "results": [
                {"text": "أيوه المشكلة اتحلت تمام وشغال كويس", "confidence": "0.95"}
            ]
        }
    }

    resp = client.post(
        f"/api/v1/telephony/vonage/speech?followup_id={followup.id}&call_id={call.id}&attempt=1",
        json=payload,
    )
    assert resp.status_code == 200
    ncco = resp.json()
...
def test_vonage_speech_webhook_no(client):
    ...
    db = SessionLocal()
    customer = Customer(name="Vonage Test User 2", phone="01022223333")
    db.add(customer)
    db.commit()

    case = Case(customer_id=customer.id, issue="بطء سرعة التحميل", status=CaseStatus.AI_FOLLOW_UP_SCHEDULED)
    db.add(case)
    db.commit()
    ...
    payload = {
        "speech": {
            "results": [
                {"text": "لأ لسه المشكلة موجودة ومش شغال", "confidence": "0.92"}
            ]
        }
    }
...
def test_vonage_event_webhook_failure(client):
    ...
    db = SessionLocal()
    customer = Customer(name="Vonage Test User 3", phone="01033334444")
    db.add(customer)
    db.commit()

    case = Case(customer_id=customer.id, issue="انقطاع متكرر", status=CaseStatus.AI_FOLLOW_UP_SCHEDULED)
    db.add(case)
    db.commit()
    ...
    resp = client.post(
        f"/api/v1/telephony/vonage/event?followup_id={followup.id}&call_id={call.id}",
        json={"status": "busy", "duration": "0"},
    )
    assert resp.status_code == 200
